...
describe('destino de cada categoría', () => {
  ...
  it('ningún punto hostil llega al sitio, y el limpio sí', () => {
    ...
    for (const d of ['frenada', 'senuelo', 'registrada'] as const) expect(alcance(d)).toBeLessThan(SITIO_X)
...
describe('reparto real', () => {
  it('normaliza a pesos que suman 1, ordenados de mayor a menor, aceptando conteos en texto', () => {
    ...
    expect(r[0]).toMatchObject({ categoria: 'recon_cms', destino: 'registrada' })
    expect(r[1].peso).toBeCloseTo(300 / 1000)
    for (let i = 1; i < r.length; i++) expect(r[i].peso).toBeLessThanOrEqual(r[i - 1].peso)
  })
  it('sin actividad hostil no hay reparto (y entonces todo punto es limpio)', () => {
    expect(repartoHostil([])).toEqual([])
    expect(repartoHostil([{ category: 'x', count: 0 }, { category: 'y', count: 'nan' }])).toEqual([])
    const azar = crearAzar(1)
    for (let i = 0; i < 50; i++) expect(nuevoTipo([], azar).destino).toBe('limpia')
  })
  it('el resumen de capas suma frenadas y señuelos desde los mismos agregados', () => {
    expect(resumenCapas(filas, 1000, 17)).toEqual({ detectados: 1000, frenadas: 80, senuelos: 20, bloqueos: 17 })
  })
...
describe('azar con semilla', () => {
  ...
  it('elegirPonderado respeta los extremos y descarta pesos no positivos', () => {
    const items = [{ id: 'a', peso: 0 }, { id: 'b', peso: 1 }, { id: 'c', peso: 3 }]
...
describe('escena estática del servidor', () => {
  ...
  it('cada punto está en un tramo que su destino recorre de verdad', () => {
    for (const p of escena) {
      ...
      if (p.destino === 'registrada') expect(p.x).toBeLessThan(CAPAS_X[1] + 0.11)
...
describe('bitácora', () => {
  it('los tramos cubren exactamente el reparto y agrupan la cola fina', () => {
    const reparto = repartoHostil([...filas, { category: 'protocol_anomaly', count: 1 }])
    ...
    for (let i = 1; i < tramos.length; i++) expect(tramos[i].desde).toBeCloseTo(tramos[i - 1].desde + tramos[i - 1].ancho, 10)
  })
})
