...
describe('locsFromSitemap', () => {
  it('extrae todas las URLs <loc> y las recorta', () => {
    const xml = `<?xml version="1.0"?><urlset>
      <url><loc>https://codebymike.net/</loc></url>
      <url><loc> https://codebymike.net/notes </loc><lastmod>2026-07-09</lastmod></url>
    </urlset>`
    expect(locsFromSitemap(xml)).toEqual([
      'https://codebymike.net/',
      'https://codebymike.net/notes',
    ])
  })
...
describe('submitToIndexNow', () => {
  it('no llama a fetch si no hay URLs', async () => {
    ...
    const r = await submitToIndexNow('https://codebymike.net', [])
  ...
  it('envía host, clave y keyLocation correctos', async () => {
    ...
    expect(body.keyLocation).toBe(`https://codebymike.net/${INDEXNOW_KEY}.txt`)
    expect(body.urlList).toEqual(['https://codebymike.net/notes'])
  })

  it('trata 202 (clave pendiente) como éxito', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => new Response(null, { status: 202 })))
    const r = await submitToIndexNow('https://codebymike.net', ['https://codebymike.net/'])
...
describe('submitSitemapToIndexNow', () => {
  it('lee el sitemap propio y reenvía sus URLs', async () => {
    const xml = '<urlset><url><loc>https://codebymike.net/</loc></url></urlset>'
