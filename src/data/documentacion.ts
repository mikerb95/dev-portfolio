...
export const REQUISITOS_FUNCIONALES: Modulo[] = [
  {
    ...
    items: [
      ...
      { id: 'RF-013', titulo: 'Versión en inglés del sitio público', descripcion: 'Las páginas públicas de marca existen bajo el prefijo /en con contenido traducido, selector de idioma que conserva la página actual y hreflang recíproco. El español sigue siendo el idioma canónico, sin prefijo.', prioridad: 'media', estado: 'parcial', origen: 'src/i18n/ (config, routing, format, es, en), src/pages/en/, src/middleware.ts', verificacion: '197 tests entre tests/i18n-routing.test.ts, tests/i18n-routing-guards.test.ts y tests/i18n-dictionary.test.ts (paridad de claves ES/EN, ida y vuelta de rutas y casos adversariales del prefijo); npx astro check rompe si una clave existe en un diccionario y no en el otro.', notas: 'Traducidas hoy: home, engineering, tools, security, contact, certifications y architecture, más el chrome global (navbar, footer, 404) y el feed /en/rss.xml. Una sola implementación por página: la variante /en es un cascarón de tres líneas que reexporta la misma página, el locale sale de la URL del request. TRANSLATED_ROUTES en src/i18n/routing.ts es la única fuente de verdad de qué existe en inglés: los enlaces, el sitemap y el hreflang solo apuntan a /en cuando la página está en esa lista, y el middleware redirige el resto de /en/* a su versión en español en vez de dejar un 404. Pendientes: notas técnicas, /docs, LAB, contenido en base de datos y assets (OG y CV en inglés).', relacionados: ['RF-007', 'RF-014', 'RNF-20', 'RNF-21'] },
