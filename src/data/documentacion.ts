...
export const REQUISITOS_FUNCIONALES: Modulo[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'RF-807', titulo: 'Actualización en vivo del portal', descripcion: 'La información del portal (notificaciones, mensajes, facturas, salud del proyecto) se refresca sola mientras la pestaña está visible, sin recargar.', prioridad: 'media', estado: 'planeado', notas: 'Diseño cerrado en docs/plan-portal-tiempo-real.md: polling de un digest barato cada 20 s, no SSE ni WebSockets — Turso no tiene pub/sub, así que el servidor tendría que sondear igual, pagando además la conexión abierta. Pausa con la pestaña oculta, backoff ante error y fail-open silencioso.', relacionados: ['RF-805'] },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'RF-505', titulo: 'Pruebas de carga (k6)', descripcion: 'Escenarios de carga automatizados contra endpoints críticos, integrados al pipeline.', prioridad: 'media', estado: 'planeado', notas: 'Fase 5 del plan LAB (docs/plan-lab.md); es la última fase pendiente. Bloqueada por un target de preview/staging estable, que a su vez depende de VERCEL_TOKEN en GitHub Secrets.' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'RF-706', titulo: 'Diagramas BPMN de los procesos de negocio', descripcion: 'Los cuatro procesos de negocio (cobro de campo, acceso al portal de clientes, respuesta de seguridad y ciclo de monitoreo) modelados en notación BPMN con carriles por participante, compuertas y eventos, navegables desde /docs.', prioridad: 'media', estado: 'implementado', origen: 'src/pages/docs/diagrama-bpmn.astro, src/data/bpmn.ts, src/lib/bpmn-layout.ts, src/components/Bpmn{Diagram,Shape}.astro', verificacion: 'tests/bpmn.test.ts (57 casos) verifica la geometría, no solo que no lance: ninguna flecha atraviesa una figura ajena, ningún nodo comparte celda, ninguna etiqueta se encima con otra, toda etiqueta cabe en su figura, toda compuerta abre al menos dos ramas etiquetadas, todo nodo es alcanzable desde el inicio y todo camino termina en un evento de fin. Tres casos hostiles confirman que el detector no es decorativo.', notas: 'Se descartó bpmn-js (bpmn.io) para no meter ~500 KB de dependencia de UI por cuatro diagramas: el SVG se genera en el servidor desde un modelo tipado, sin JavaScript en el cliente. La verificación automática de la geometría salió cara pero se pagó sola — encontró tres flechas que cruzaban cajas, una etiqueta de compuerta encimada con la de su rama, dos ramas cuyos "sí"/"no" caían en el mismo punto (leyéndose sobre el camino contrario) y una tarea sin salida en el modelo de seguridad.', relacionados: ['RF-703', 'RF-306', 'RF-801', 'RF-603', 'RF-401'] },
