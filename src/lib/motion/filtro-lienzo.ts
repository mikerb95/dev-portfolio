...
export function montarFiltro(
  ...
  let dpr = 1;
  let tuNacio = false;
  const finoPuntero = window.matchMedia(
  ...
  const avanzar = (dt: number) => {
    ...
    for (const p of particulas) {
      ...
      if (p.estado === "viaje") {
        ...
        for (let i = 0; i < CAPAS_X.length; i++)
          if (antes < CAPAS_X[i] && p.x >= CAPAS_X[i]) cruzar(p, i);
        if (
          p.destino === "registrada" &&
          p.marcada &&
          p.x >= alcance("registrada")
        ) {
          p.estado = "caida";
          p.vy = 0.02;
          p.t = 0;
    ...
    for (let i = 0; i < brilloTramo.length; i++)
      brilloTramo[i] = Math.max(0, brilloTramo[i] - dt * 1.6);
  ...
  const dibujar = () => {
    ...
    for (const b of bloqueos) {
      ...
      const x =
        (1 - e) * (1 - e) * b.x0 + 2 * (1 - e) * e * cxp + e * e * CAPAS_X[1];
  ...
  if (pista) {
    pista.textContent = finoPuntero
      ? datos.textos.pista
      : datos.textos.pistaTactil;
    pista.hidden = false;
  }
