...
export const CATEGORIAS_FRENADAS: readonly string[] = [
  "blocklist",
  "api_abuse",
];
...
export type Reparto = {
  categoria: string;
  peso: number;
  destino: Exclude<Destino, "limpia">;
}[];
...
export function elegirPonderado<T extends { peso: number }>(
  items: readonly T[],
  r: number,
): T | null {
...
export function nuevoTipo(
  reparto: Reparto,
  azar: () => number,
): { destino: Destino; categoria: string | null } {
  if (reparto.length === 0 || azar() >= FRACCION_HOSTIL)
    return { destino: "limpia", categoria: null };
...
export function crearOnda(segmentos: number): Onda {
  return {
    y: new Float32Array(segmentos + 1),
    v: new Float32Array(segmentos + 1),
  };
}
 ...
 */
export function escenaEstatica(
  reparto: Reparto,
  semilla: number,
  cantidad: number,
): PuntoEstatico[] {
 ...
 */
export function tramosBitacora(
  reparto: Reparto,
  minimo = 0.025,
): { categoria: string; desde: number; ancho: number; destino: Destino }[] {
  const tramos: {
    categoria: string;
    desde: number;
    ancho: number;
