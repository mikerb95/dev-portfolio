...
function guionActual(ts) {
  ...
  runInContext(
    datos + "\n;__salida = { GUION_INTRO, GUION_BEATS, GUION_OUTRO };",
    ctx,
    {
      timeout: 5000,
    },
...
function alinear(mazo, previos) {
  ...
  const huerfanos = previos
    .filter((n) => !usados.has(clave(n.titulo)))
    .map((n) => n.titulo);
...
if (check) {
  if (salida !== ts) {
    console.error(
      "El guion no coincide con el mazo. Corre `npm run guion:sync`.",
    );
    process.exit(1);
  }
  ...
  if (nuevos.length)
    console.log(
      `  nuevos, con notas del mazo por reescribir:\n    - ${nuevos.join("\n    - ")}`,
    );
  if (huerfanos.length)
    console.log(
      `  descartados (ya no están en el mazo):\n    - ${huerfanos.join("\n    - ")}`,
    );
}
