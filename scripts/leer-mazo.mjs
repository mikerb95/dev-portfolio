...
export function plantillaDe(html) {
  ...
  if (i === -1)
    throw new Error('no hay <script type="__bundler/template"> en el bundle');
...
export function bloqueDeDatos(fuente) {
  ...
  if (ini === -1 || marcaBeats === -1)
    throw new Error("el mazo no declara NODES/BEATS como se esperaba");
...
export function beatsDe(html) {
  ...
  if (!Array.isArray(beats) || beats.length === 0)
    throw new Error("BEATS salió vacío");
